		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count != f34->blocksize) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Incorrect block size (%d), expected size = %d\n",
			__func__, count, f34->blocksize);
		return -EINVAL;
	}
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not read data from 0x%04x\n", __func__,
				f34->data_base_addr + DATA_BLOCK_DATA_OFFSET);
		return retval;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count) {
		...
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
				"%s Could not write data to 0x%04x\n", __func__,
				f34->data_base_addr + DATA_BLOCK_DATA_OFFSET);
			return retval;
		}
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not write bootloader ID to 0x%04x\n",
			__func__, f34->data_base_addr + DATA_BLOCK_DATA_OFFSET);
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case F34_CMD_ENABLE_FLASH_PROG:
		...
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not write command 0x%02x to 0x%04x\n",
			__func__, f34->cmd,
			f34->data_base_addr + DATA_FLASH_COMMAND_OFFSET);
			return retval;
		}
		...
		break;
	default:
		dev_err(&rmi4_data->i2c_client->dev, "%s Unknown command 0x%02x\n",
				__func__, f34->cmd);
		count = -EINVAL;
	break;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not write block number to 0x%04x\n", __func__,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!f34->inflashprogmode) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Not in flash programming mode cannot rescan PDT\n",
			__func__);
		return -EINVAL;
	}
	...
	if (rescan) {
		...
		if (!f01found || !f34found) {
			dev_err(&rmi4_data->i2c_client->dev,
				"%s Fail to find both F01 and F34 ", __func__);
			return -EINVAL;
		}
	}
	...
	struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not read command from 0x%04x\n", __func__,
			f34->data_base_addr + DATA_FLASH_COMMAND_OFFSET);
	}
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Could not read status from 0x%04x\n", __func__,
...
static int synaptics_rmi4_f34_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!f34) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Failed to alloc for synaptics_rmi4_f34_handle\n",
			__func__);
		retval = -ENOMEM;
		goto exit;
	}
	...
	for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
		retval = f34->fn_ptr->read(rmi4_data, ii,
	...
	for (ii = intr_offset;
		ii < ((rmi_fd.intr_src_count & 0x07) + intr_offset); ii++) {
		f34->intr_mask |= 1 << ii;
	}
	...
	retval = f34->fn_ptr->read(rmi4_data,
			f34->query_base_addr + QUERY_BLOCK_SIZE_OFFSET, buf, 2);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s Failed to create sysfs file for F34 data\n",
			__func__);
		goto exit_free_mem;
	}
